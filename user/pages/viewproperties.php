<script>
  function acc(iden,action){
     alert('yes');
    var userid = "<?php echo $sess_userid; ?>";

    var name = "<?php echo $sess_username; ?>";
    var dataString = "id="+iden+"&userid="+userid+"&name="+name+"&action="+action;
    var status = $("#status").val();
    var identity = $('#loading')+iden;
    var confirms = confirm("Clicking this will send an acquire on demand message to the admin, are you sure you want to continue?");
    if (confirms == true) {
    $.ajax({
        type: "POST",
        url: "control.php",
  ...
  foreach ($fetch as $key) {
    ...
    ?>
                        ...
                        <tr>
                          ...
                          <td><?php echo $date; ?></td>
                          <td>
                            <a style="cursor:pointer" onclick= "acc('<?php echo $id ?>', 'acquire')" title="acquire" id="del<?php echo $id?>">Acquire</a> <span id="loading<?php echo $id ?>" style="display:none"><img src="../img/core-img/loading.gif"></span>
                          </td>
