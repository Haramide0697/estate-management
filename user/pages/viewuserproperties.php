...
<?php
$query = $conn->query("SELECT * FROM property_userdemand order by 'id' DESC");
...
$query = $conn->query("SELECT * FROM staff WHERE id = $userid");
$fetch = $query->fetchAll(PDO::FETCH_OBJ);
$count = $query->rowCount();
$num = 1;
if ($count > 0) {
  foreach ($fetch as $key2) {
    $surname = $key2->surname;
    $othernames = $key2->othernames;
    $gender = $key2->gender;
  ...
  foreach ($fetchh as $value) {
    ...
    ?>
                        ...
                        <tr>
                          ...
                          <?php
                          $queryy2 = $conn->query("SELECT * FROM reply_userpmessages WHERE message_id = $id");
                          ...
                          ?>
                          <td>
                          <a href="files/<?php echo $rmess ?>" download><button class="btn btn-success"> Download Document </button></a></td>     
                          ...
                          <td><?php echo $date; ?></td>
                          <td>
                            <a style="cursor:pointer" title="Accept" onclick="reply('<?php echo $id ?>','<?php echo $userid ?>');" data-toggle="modal" data-target="#<?php echo $use ?><?php echo $id; ?>">Accept</a> <span id="loading<?php echo $id ?>" style="display:none"><img src="../img/core-img/loading.gif"></span>
                          </td>
                    ...
                    <div class="modal-content">
                     ...
                     <div class="modal-body">
                        <form method="post" enctype="multipart/form-data" id="replymess<?php echo $id ?>">
                          <input type="number" id="iden" name="identity1" value="<?php echo $id ?>" style="display: none;">
                          <input type="number" id="iden" name="userid" value="<?php echo $userid ?>" style="display: none;">
                          <input type="number" id="iden" name="property_id" value="<?php echo $property_id ?>" style="display: none;">
                          <div class="form-group">
                            <label for="reply"> Input Document </label>
                            <input type="file" name="reply" id="reply">
                          </div>
